...
#define READVECTOR(vec) {                       \
    ...
    namespace {
        ...
        struct FileIOReader : IOReader {
            ...
            int64_t operator()(
                    void *ptr, int64_t size, int64_t nitems) override {
        ...
        struct FileIOWriter : IOWriter {
            ...
            int64_t operator()(
                    const void *ptr, int64_t size, int64_t nitems) override {
 ...
 **************************************************************/
    ...
    VectorTransform *read_VectorTransform(IOReader *f) {
        ...
        if (h == fourcc("rrot") || h == fourcc("PCAm") ||
            ...
            if (h == fourcc("rrot")) {
                ...
                if (h == fourcc("PcAm")) READ1 (pca->balanced_bins);
                READVECTOR (pca->mean);
    ...
    static void read_ivf_header(
        ...
        if (ids) { // used in legacy "Iv" formats
            ...
            for (int64_t i = 0; i < ivf->nlist; i++) READVECTOR ((*ids)[i]);
        }
...
// used for legacy formats
    ...
    static IndexIVFPQ *read_ivfpq(IOReader *f, uint32_t h, int io_flags) {
        ...
        if (legacy) {
            ...
            for (int64_t i = 0; i < ail->nlist; i++) READVECTOR (ail->codes[i]);
        } else {
...
#define TRYCLONE(classname, obj) \
    ...
    VectorTransform *Cloner::clone_VectorTransform(const VectorTransform *vt) {
        TRYCLONE (RemapDimensionsTransform, vt) TRYCLONE (OPQMatrix, vt) TRYCLONE (PCAMatrix,
                                                                                   vt) TRYCLONE (
                RandomRotationMatrix, vt) TRYCLONE (LinearTransform, vt) {
    ...
    IndexIVF *Cloner::clone_IndexIVF(const IndexIVF *ivf) {
        TRYCLONE (IndexIVFPQR, ivf) TRYCLONE (IndexIVFPQ, ivf) TRYCLONE (IndexIVFFlat,
                                                                         ivf) TRYCLONE (
                IndexIVFScalarQuantizer, ivf) {
    ...
    Index *Cloner::clone_Index(const Index *index) {
        TRYCLONE (IndexPQ, index) TRYCLONE (IndexLSH, index) TRYCLONE (IndexFlatL2,
                                                                       index) TRYCLONE (IndexFlatIP,
                                                                                        index) TRYCLONE (
                IndexFlat, index) TRYCLONE (IndexScalarQuantizer, index) TRYCLONE (
                MultiIndexQuantizer,
                index) if (const IndexIVF *ivf = dynamic_cast<const IndexIVF *>(index)) {
