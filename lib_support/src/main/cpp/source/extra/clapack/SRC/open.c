...
#endif
{
...
#endif
    f__external = 1;
    if (a->ounit >= MXUNIT || a->ounit < 0) err(a->oerr, 101, "open")
    if (!f__init)
        f_init();
    ...
    switch (a->osta ? *a->osta : 'u') {
        ...
        case 'O':
            ...
            fclose(tf);
#else
            if (access(buf, 0)) opnerr(a->oerr, errno, "open")
#endif
            break;
        case 's':
        case 'S':
            ...
            goto replace;
#else
            if (!(b->ufd = tmpfile())) opnerr(a->oerr, errno, "open")
            b->ufnm = 0;
#ifndef NON_UNIX_STDIO
            b->uinode = b->udev = -1;
#endif
...
#else
            if (!access(buf, 0)) opnerr(a->oerr, 128, "open")
#endif
            /* no break */
    ...
    if (!(tf = FOPEN(buf, f__w_mode[ufmt | 2]))) {
        if (tf = FOPEN(buf, f__r_mode[ufmt]))
            ...
            b->uwrt = 1;
            b->urw = 2;
        } else err(a->oerr, errno, "open");
    }
    b->useek = f__canseek(b->ufd = tf);
#ifndef NON_UNIX_STDIO
    if ((b->uinode = f__inode(buf, &b->udev)) == -1) opnerr(a->oerr, 108, "open")
#endif
    if (b->useek)
        if (a->orl)
            rewind(b->ufd);
